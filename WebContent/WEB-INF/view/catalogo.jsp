...
<%  UtenteBean utenteSessione = (UtenteBean) session.getAttribute("utente");
	boolean amministratore = utente != null && "amministratore".equals(utenteSessione.getRuolo());
...
%>
    <div class="box-catalogo-vuoto">
        <h4>Il catalogo è vuoto</h4>
      	...
      	<%if(amministratore) {%>
        	<h5><a href="<%=request.getContextPath()%>/aggiungiModificaProdotto">Clicca qui</a> per aggiungere un prodotto</h5>
        <%} %>
    </div>
<% 
    } else { 
                    ...
                    <tr>
                        ...
                        <td class="container-azioni">
                        	<a class="azione azione-visualizza"
                        	   href="<%=request.getContextPath()%>/visualizzaProdotto?id=<%=prodotto.getId()%>">
							...
							<%if(amministratore) { %>
								...
								<a class="azione azione-modifica"
									href="<%=request.getContextPath()%>/aggiungiModificaProdotto?id=<%=prodotto.getId()%>">
									<svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24" fill="none">
							...
							<%} else { %>
								<a class="azione azione-carrello" onclick="aggiungiAlCarrello(<%=prodotto.getId()%>, '<%=request.getContextPath()%>')">
